...
## Perubahan lokal BELUM di-push (2026-09-09 malam, menunggu perintah user)
- **Laporan mingguan → harian EOD** (permintaan user): `report.yml` cron "7 16 * * 1-5" + cadangan "53 16" (23:07 WIB Sen–Jum, setelah grid daily terakhir); `report.py` → `daily-YYYY-MM-DD.pdf`, banner/judul "LAPORAN HARIAN", KEEP 52→90, ringkasan eksekutif kini OHLC hari itu (WIB) + 7 hari, **section 7 baru "Hasil rekomendasi per hari"** (tabel 7 hari dari log EOD tracking.json + detail entry hari terbaru: pola/bias/hasil/MFE-MAE/narasi), section event → "7 hari ke depan" (no. 8), berita → no. 9. Frontend: label panel Backtest + kartu Jadwal → "Laporan harian PDF (EOD)" (grid spec 23:07 tradingDaysOnly). Smoke test + pypdf (section baru ADA, tabel per-hari terisi data nyata: 2026-09-08 net +1 TP1, 2026-09-06 net −1 SL) + tsc + build LULUS. PDF mingguan lama tetap di repo (arsip, index tetap valid).
- Batch backtest jujur + layout Riwayat mobile: SUDAH di-push `9b8689b`.
